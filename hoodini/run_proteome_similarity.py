...
def compute_wgrr(hits_df: pl.DataFrame | pl.LazyFrame,
                 ...
                 symmetric: bool = True) -> pl.DataFrame:
    """
    ...
    Returns Polars DataFrame with columns: ["A", "B", "wGRR_sym", "AAI"]
    """
    ...
    if rbh.is_empty():
        return pl.DataFrame(columns=["A", "B", "wGRR_sym", "AAI"])
    ...
    return w.select(["A", "B", "wGRR_sym", "AAI"])


def compute_aai_rbh(hits_df: pl.DataFrame | pl.LazyFrame,
                    proteins_df: pl.DataFrame | pl.LazyFrame,
                    pident_min: Optional[float] = 30.0,
                    exclude_self: bool = True) -> pl.DataFrame:
    """
    ...
    rbh = _rbh_from_ann(ann)
    if rbh.is_empty():
        return pl.DataFrame(columns=["A", "B", "AAI", "n_RBH", "RBH_frac_min"])
...
def compute_vcontact2_hypergeom(proteins_df: pl.DataFrame | pl.LazyFrame,
    ...
    if not fam_like:
        return pl.DataFrame(columns=["A", "B", "k", "K_A", "K_B", "M", "pval", "p_adj", "score"])
    ...
    if pairs.is_empty():
        return pl.DataFrame(columns=["A", "B", "k", "K_A", "K_B", "M", "pval", "p_adj", "score", "AAI"])
    ...
    try:
        ...
        if _HAS_SCIPY:
            ...
            pvals = np.array([hypergeom_sf(k, Mv, Ka_i, Kb_i) for k, Ka_i, Kb_i, Mv in zip(k_vals, Ka, Kb, Mvals)])
    except Exception:
...
def run_proteome_similarity(all_prots: Union[pl.DataFrame, pl.DataFrame, pl.LazyFrame],
    ...
    try:
        if 'unique_id' in set(all_prots_pl.columns):
            ...
            if {"temp_seqid", "seqid", "unique_id"}.intersection(all_neigh_pl.columns):
                an_pl = all_neigh_pl.select([c for c in ["temp_seqid", "seqid", "unique_id"] if c in all_neigh_pl.columns])
                mapping = {}
                ...
                if "seqid" in an_pl.columns:
                    seq_group = (
                        an_pl
                        .filter(pl.col("seqid").is_not_null() & pl.col("unique_id").is_not_null())
                        .group_by("seqid")
                        .agg([
                ...
                if mapping:
                    ...
                    all_prots_pl = all_prots_pl.with_columns(
                        pl.col("target_nuc").cast(pl.Utf8).replace(mapping, default=None).alias("target_nuc_mapped")
                    ).with_columns(
    ...
    if outdir:
        ...
        if wgrr_df is not None:
            wgrr_df.write_csv(os.path.join(outdir, "wgrr.tsv"), separator="\t", include_header=False)
        if aai_df is not None:
            aai_df.write_csv(os.path.join(outdir, "aai.tsv"), separator="\t", include_header=False)
        if vcon_df is not None:
            vcon_df.write_csv(os.path.join(outdir, "vcontact_hypergeom.tsv"), separator="\t", include_header=False)
    ...
    if mode == "wgrr":
        ...
        if wgrr_df is None:
            wgrr_df = pl.DataFrame(columns=["A", "B", "wGRR_sym", "AAI"])
        if aai_df is None:
            aai_df = pl.DataFrame(columns=["A", "B", "AAI", "n_RBH", "RBH_frac_min"])
        if vcon_df is None:
            vcon_df = pl.DataFrame(columns=["A", "B", "k", "K_A", "K_B", "M", "pval", "p_adj", "score", "AAI"])
