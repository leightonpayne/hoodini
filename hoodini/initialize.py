...
def initialize_inputs(
    *,
    ...
    if output:
        ...
        console.print("[bold red]Error:[/bold red] Either `input_path` or `inputsheet` must be provided.")
        sys.exit(1)
...
def check_assembly_db() -> None:
    ...
    try:
        ...
        if summary_path.exists():
            ...
            age = now - mtime
            console.print(f"📁 Assembly DB found: [bold]{summary_path}[/bold] (updated: {mtime:%Y-%m-%d})")
            if age > one_month:
                console.print(f"[bold yellow]⚠️  WARNING: The assembly database is older than 30 days.[/bold yellow]")
                console.print("[yellow]To update the database, run: [bold]hoodini update assembly_summary[/bold][/yellow]\n")
            else:
            ...
            console.print(f"[green]✔️  Downloaded assembly database to [bold]{output_path}[/bold].[/green]\n")
            return
