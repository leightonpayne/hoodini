...
def download_with_aria2c(urls, dest_dir, connections=16, split=16, show_progress=True, show_aria2c_output=False, out_names=None, num_threads: int = 0):
    ...
    cmd = [
        "aria2c", "--summary-interval=1", "--enable-color=false",
        "--max-connection-per-server", max_conn,
        "--split", max_conn,
        "-k", "1M",
        "-d", str(dest_dir), "-i", input_file
    ]
        ...
        if show_progress:
            ...
            while True:
                ...
                for part in parts[:-1]:
                    ...
                    if show_aria2c_output and (downloaded is not None or total is not None or pct is not None):
            ...
            if total_bytes:
                progress.update(task, total=total_bytes, completed=total_bytes,
