...
def run_nt_links(pairwise_ani: pl.DataFrame, output_dir: str, all_neigh: Optional[pl.DataFrame] = None, threads: int = 8, evalue: Optional[float] = None, keep_temp: bool = False) -> pl.DataFrame:
    ...
    if 'Align_fraction_ref' in df.columns or 'Align_fraction_query' in df.columns:
        df = df.dropna(subset=[c for c in ['Align_fraction_ref', 'Align_fraction_query'] if c in df.columns])
    ...
    for row in iterator:
        try:
            q_name = getattr(row, q_col, None) if hasattr(row, q_col) else row[ df.columns.get_loc(q_col) ]
        except Exception:
            q_name = row[ df.columns.get_loc(q_col) ] if q_col in df.columns else None
        try:
            r_name = getattr(row, r_col, None) if hasattr(row, r_col) else row[ df.columns.get_loc(r_col) ]
        except Exception:
        ...
        if out_file.exists():
            ...
            if not visual_file.exists():
                console.log(f"fastANI did not create .visual for {pair_key}, expected {visual_file}")
                continue
