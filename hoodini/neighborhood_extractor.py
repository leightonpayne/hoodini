    ...
    if nucleotide_id:
        for rec in records_list:
            ...
            if nucleotide_id in rec_version:
                ...
                break
        if record is None:
            raise NeighborhoodExtractionError(f"Nucleotide {nucleotide_id} not found in {gbff_path}. Available: {[getattr(r, 'version', '?') for r in records_list[:3]]}")
    else:
...
def parse_gff_faa(gff_path: str, faa_path: str) -> pl.DataFrame:
    ...
    try:
        ...
        gff = pl.read_csv(gff_path, separator="\t", has_header=False, new_columns=gff_header, skip_rows_after_header=0)
    ...
    gff = gff.with_columns(
        pl.col("attributes").str.extract(r"ID=([^;]+)", 1).alias("protein_id")
    )
...
def extract_features_in_window(
    ...
    subset = features_df.filter(
        (pl.col("start") >= start_win) & (pl.col("end") <= end_win)
    )
    ...
    columns = ['seqid', 'source', 'type', 'start', 'end', 'score', 'strand', 'phase', 'protein_id', 'id', 'sequence']
...
def annotate_sorfs(
    ...
    for i, pred in enumerate(orf_finder.find_genes(sequence)):
        ...
        for row in features_df.iter_rows(named=True):
            overlap_pct = _calculate_overlap(row['start'], row['end'], pred.begin + start_win, pred.end + start_win)
            if overlap_pct > 10:
                overlap_flag = True
                break
    ...
    for i, (start, stop, strand, description) in enumerate(orfipy_core.orfs(seq_upper, minlen=100, maxlen=1000, partial3=False, between_stops=False)):
        overlap_flag = False
        for row in features_df.iter_rows(named=True):
            overlap_pct = _calculate_overlap(row['start'], row['end'], start + start_win, stop + start_win)
            if overlap_pct > 0:
                overlap_flag = True
                break
    ...
    try:
        ...
        if gbf_file:
            ...
            if fna_file and os.path.exists(fna_file):
                ...
                seq_match = fna_df.filter(pl.col("id") == nucleotide_id) if nucleotide_id else fna_df.head(1)
            ...
            raise NeighborhoodExtractionError("Must provide either gbf_file or (gff_file and faa_file)")
        ...
        if sorfs and window_sequence:
            subgff = annotate_sorfs(subgff, window_sequence, start_win, nucleotide_id or record_version, unique_id or "unknown")
