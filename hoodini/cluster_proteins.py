...
def cluster_proteins(
    ...
    rep_to_fam = (
        ...
        .with_columns(
            pl.int_range(pl.len()).add(1).cast(pl.Int64).alias("fam_cluster")
        )
    ...
    if sorfs:
        ...
        clusterdf_pl = clusterdf_pl.filter(
            ~(pl.col("fam_cluster").count().over("fam_cluster") == pl.col("member").str.contains("orfipy").sum().over("fam_cluster"))
        )

    # Join with all_prots
    result = all_prots.join(
        clusterdf_pl.select(["member", "fam_cluster"]),
        left_on="id",
        right_on="member",
        how="left"
    )
