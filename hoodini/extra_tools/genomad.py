...
def run_genomad(all_neigh, output, num_threads, valid_unique_ids):
    ...
    genomad_command = [
        ...
        "--splits", str(max(1, num_threads or 1)),
        neighborhood_fasta,
    ...
    plasmid_prots = pl.read_csv(plasmid_file, separator="\t") if os.path.exists(plasmid_file) else pl.DataFrame()
    virus_prots = pl.read_csv(virus_file, separator="\t") if os.path.exists(virus_file) else pl.DataFrame()
