...
def run_padloc(all_gff, all_prots, output, num_threads):
    ...
    temp_gff = temp_gff.join(
        all_prots.select(["id", "unique_id", "sequence"]),
        on="id", 
        how="left"
    )
    ...
    temp_gff = temp_gff.with_columns(
        ("ID=" + pl.col("temp_id")).alias("attributes")
    )
    # Keep only rows that can produce valid FASTA entries
    temp_gff = temp_gff.filter(
        pl.col("temp_id").is_not_null()
    ...
    # Write GFF
    temp_gff_out = temp_gff.select(["seqid", "source", "type", "start", "end", "score", "strand", "phase", "attributes"])
    temp_gff_out.write_csv(f"{output}/temp.gff", separator="\t", include_header=False)
    
    # Write FASTA
    temp_fasta = (
        temp_gff
        .select(["temp_id", "sequence"]) 
        .filter(
            pl.col("temp_id").is_not_null()
            & pl.col("sequence").is_not_null()
        )
    ...
    with open(f"{output}/temp.fasta", "w") as f:
