...
def run_emapper(all_prots: pl.DataFrame, output: str, num_threads: int = 1) -> pl.DataFrame:
    ...
    emapper_dir = os.path.join(output, "emapper")
    os.makedirs(emapper_dir, exist_ok=True)

    fasta_path = os.path.join(output, "results.faa")
    fasta_fallback = os.path.join(output, "results.fasta")

    # write FASTA if missing
    if not os.path.exists(fasta_path):
        if os.path.exists(fasta_fallback):
    ...
    results_m8 = os.path.join(emapper_dir, "results.m8")
    tmpdir = os.path.join(emapper_dir, "mmseqs_tmp")
    ...
    def run_mmseqs(db_prefix: str, use_gpu: bool):
        cmd = [
            ...
            results_m8,
            tmpdir,
            "--threads", str(max(1, int(num_threads or 1))),
        ]
        if use_gpu:
            cmd += ["--gpu", "1"]
    ...
    hits_all = pl.read_csv(
        ...
        separator="\t",
        new_columns=[
            "qseqid","sseqid","pident","alnlen","mismatch",
